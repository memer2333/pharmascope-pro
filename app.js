...
async function performSearch(query) {
  ...
  var results = await searchDrugs(query).catch(function() { return []; });
  if (!results || results.length === 0) {
    mc.innerHTML = '<div class="empty-state"><p>No results found for "' + escapeHtml(query) + '". Try a generic name or brand name.</p><button onclick="renderHomePage()">Back to Home</button></div>';
    return;
  }
...
async function selectDrug(dbResult) {
  if (!dbResult) return;
  var drugName = dbResult.name || 'Unknown';
  var localData = getDrugLocalData(drugName);
  ...
  safeApiCall(function() { return getFullDrugLabel(drugName); }, null, 6000).then(function(dailyMedLabel) {
    if (!dailyMedLabel) return;
    AppState.currentDailyMedLabel = dailyMedLabel;
    var enriched = renderDrugProfile(dbResult, localData, dailyMedLabel, null, null);
...
async function loadSuggestions(query) {
  ...
  var results = await getSuggestions(query).catch(function() { return []; });
  ...
  box.innerHTML = results.map(function(r) {
    return '<div class="suggestion-item" onclick="performSearch(\'' + escapeAttr(r.name) + '\')"><span>' + escapeHtml(r.name) + '</span><small>' + escapeHtml(r.type || 'Drug') + '</small></div>';
  }).join('');
...
function renderHomePage() {
  ...
  mc.innerHTML = '<div class="home-container">' +
    ...
    '<div class="feature-card"><div class="feature-icon">&#9888;&#65039;</div><h3>Adverse Events</h3><p>Review reported adverse reactions and safety data</p></div>' +
  ...
  if (homeBtn) {
    homeBtn.addEventListener('click', function() {
      var inp = document.getElementById('main-search-input');
